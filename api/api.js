...
export async function fetchDataWithoutFilter(objectId, domain, subdomain, formattedDate) {
  try {
    const response = await fetch(`${baseUrl}/eSearch?domain=${domain}&subdomain=${subdomain}&userId=${objectId}&f=createDated_D eq ${formattedDate}`);
...
export const fetchDataWithFilter = async (domain, subdomain, objectId, filterKey, filterValue, formattedDate) => {
  try {
    ...
    if (['finance', 'treatment', 'distance'].includes(filterKey)) {
      url = `${baseUrl}/eSearch?domain=${domain}&subdomain=${subdomain}&userId=${objectId}&f=${filterKey}_L eq ${parseInt(filterValue, 10)} and createDated_D eq ${formattedDate}`;
    } else {
